<template>
  <q-layout view="lHh Lpr lFf">
    ...
    <q-drawer
      ...
      dark
    >
      <q-scroll-area style="height: calc(100% - 260px); margin-top: 150px; border-right: 1px solid #ddd">
        <q-separator
          class="bg-accent"
          style="height:1px"
        ></q-separator>
        ...
        >
          ...
          <q-item-section style="margin-left:-15px">
            ...
            >{{eventChoose.eventTitle}}</q-item-label>
            <q-item-label
              class="text-green"
              caption
            >{{dateConvert(eventChoose.schedule)}}</q-item-label>
          </q-item-section>
          <q-item-section
            side
            color="indigo"
...
export default {
  ...
  computed: {
    ...
    eventChoose () {
      const myData = find(this.listOfEVENTS, ['keyIndex', this.$route.params.eventId])
      if (myData) {
        return myData
      } else {
        return ''
  ...
  methods: {
    ...
    getEvent () {
      ...
      this.$q.loading.show()
      this.$store.dispatch('event/getEventAction')
        .then(result => {
          vm.loadedBa = true
          this.$q.loading.hide()
